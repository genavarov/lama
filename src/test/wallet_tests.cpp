...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        empty_wallet();
        for (int i = 0; i < 20; i++)
            add_coin(5000000 * COIN);

        BOOST_CHECK( wallet.SelectCoinsMinConf(5000000 * COIN, 1, 1, vCoins, setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, 5000000 * COIN); // we should get the exact amount
